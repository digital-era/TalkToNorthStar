...
(function() {
  'use strict';
  // 保存原有的 switchUIStyle 函数（如果存在）
  const _originalSwitchUIStyle = typeof window.switchUIStyle === 'function' 
    ? window.switchUIStyle 
    : function() {};
  /**
   * 重写 switchUIStyle 函数
   * @param {string} style - 'modern' 或 'traditional'
   */
  window.switchUIStyle = function(style) {
    // 首先执行原有逻辑（保持原有功能）
    try {
      _originalSwitchUIStyle.call(window, style);
    } catch(e) {
      console.log('Original switchUIStyle:', e);
    }
    // 然后执行现代模式切换
    const body = document.body;
    if (style === 'modern') {
      body.classList.add('modern-style');
      body.classList.remove('traditional-style');
      // 显示现代过滤栏
      document.querySelectorAll('.modern-filter-bar').forEach(function(bar) {
        bar.style.display = 'flex';
      });
      // 初始化过滤芯片交互
      initFilterChipInteraction();
    } else {
      body.classList.remove('modern-style');
      body.classList.add('traditional-style');
      // 隐藏现代过滤栏
      document.querySelectorAll('.modern-filter-bar').forEach(function(bar) {
   ...
   * 初始化过滤芯片点击交互
   */
  function initFilterChipInteraction() {
    document.querySelectorAll('.filter-chip').forEach(function(chip) {
      // 避免重复绑定
      if (chip.dataset.initialized === 'true') return;
  ...
  function restoreUserPreference() {
    let savedStyle = 'modern'; // 默认现代风格
    try {
      savedStyle = localStorage.getItem('uiStyle') || 'modern';
    } catch(e) {
      // localStorage 不可用
    }
    // 检查下拉选择器当前值
    const selector = document.getElementById('uiStyle');
    if (selector && selector.value) {
      savedStyle = selector.value;
    }
    // 应用样式
    window.switchUIStyle(savedStyle);
  }
  // DOM 加载完成后初始化
  if (document.readyState === 'loading') {
    ...
    // 如果 DOM 已经加载完成
    setTimeout(restoreUserPreference, 0);
  }
})();
